...
const initDb = async () => {
    ...
    `);
    console.log("Table 'tasks' validée/créée avec succès !");
};

// 1. GET /api/tasks
router.get('/', async (req, res, next) => {
    try {
        await initDb(); // S'assure que la table existe avant de faire le SELECT
...
router.post('/', async (req, res, next) => {
    try {
        ...
        await initDb(); // S'assure que la table existe avant de faire le INSERT
